...
{
	public TimeSinceLabel(LabelConfig labelData, View view)
	{
	...
	public void getUpdatedTimeValue(boolean setnewDate)
	{
		...
		Date now = new Date();
		String newTime = "0 Days 0 Hours 0 Minutes";
		
		if(setnewDate)
		{
			((TimeSinceLabelConfig)labelData).setDate(dateFormat.format(now));
		}
		
		try 
		{
			newTime = TimeUtil.getDateDifference(dateFormat.parse(((TimeSinceLabelConfig)labelData).getDate()), now);
		} 
