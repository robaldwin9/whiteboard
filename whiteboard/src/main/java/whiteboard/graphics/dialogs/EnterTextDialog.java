package whiteboard.graphics.dialogs;

import java.awt.Font;
import java.awt.GridBagConstraints;
import java.awt.GridBagLayout;
import java.awt.Insets;
import java.awt.event.ActionListener;

import javax.swing.JTextField;

import whiteboard.config.Config;

public class EnterTextDialog extends WhiteboardDialog
{
	/** */
	private static final long serialVersionUID = 4575863357831177914L;
	
	/** */
	JTextField inputField;
	
	
	/** layout Objects */
	GridBagLayout layout;
	GridBagConstraints constraints;
	
	/**
	 * Constructor
	 */
	/**
	 * @param promptText
	 */
	public EnterTextDialog(String promptText, int width, int height)
	{
		
		/** layout Initialization */
		Config config = Config.getInstance();
		layout = new GridBagLayout();
		constraints = new GridBagConstraints();
		
		/** Common constraints */
		constraints.fill = GridBagConstraints.HORIZONTAL;
		constraints.weightx = 1;
		constraints.weighty = 0.5;
		constraints.gridwidth = 1;
		constraints.gridheight = 1;
		setLayout(layout);
		
		/** Add Label */
		constraints.gridx = 0;
		constraints.gridy = 0;
		constraints.anchor = GridBagConstraints.WEST;
		prompt.setText(promptText);
		this.add(prompt,constraints);
		
		/** Add Text Field */
		constraints.gridx = 2;
		constraints.insets = new Insets(1,1,1,1);
		constraints.anchor = GridBagConstraints.EAST;
		inputField = new JTextField("",500);
		inputField.setFont(new Font(config.getFontName(), Font.PLAIN, config.getDialogFontSize()));
		this.add(inputField,constraints);

		/** Add ok button */
		constraints.gridx = 0;
		constraints.gridy = 1;
		constraints.anchor = GridBagConstraints.SOUTHWEST;
		constraints.insets = new Insets(2,2,2,2);
		this.add(ok,constraints);
		
		/** Add cancel button */
		constraints.gridx =2;
		constraints.gridy = 1;
		constraints.anchor = GridBagConstraints.SOUTHEAST;
		constraints.insets = new Insets(2,2,2,2);
		this.add(cancle,constraints);
		
		this.setSize(width,height);

		setVisible(true);
	    
	}
	
	public String getUserInputText()
	{
		String userInput = "";
		userInput = inputField.getText();
		return userInput;
	}
	
	public void setokActionEvent(ActionListener e)
	{
		ok.addActionListener(e);

	}
}
